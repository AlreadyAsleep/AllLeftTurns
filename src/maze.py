...
class Maze:
    ...
    def __init__(self, size):
        ...
        self.size = size
        self.adjacency = numpy.zeroes(size, size)
    ...
    def make_maze(self):
        ...
        self.adjacency[0][random.randint(self.size)] = 1  # creates the entry to the maze
        self.adjacency[self.size][random.radint(self.size)] = 1  # creates an exit to the maze
        for i in range(1, self.size - 1):
            for j in range(self.size):
                self.adjacency[i][j] = random.randint % 2  # will place a zero if even and a one if not
